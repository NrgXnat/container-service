...
### Features
* [CS-660](https://radiologics.atlassian.net/browse/CS-660): Added optional scheduled cleanup of build directories left behind by finalized containers. Disabled by default. Configured per container server host under Administer &gt; Plugin Settings &gt; Container Service &gt; Compute Backend, with independent retention periods for completed, failed and killed containers and a configurable UTC time of day for the run. Adds POST /xapi/docker/server/build-dir-cleanup to run a cleanup pass immediately rather than waiting for the schedule. On a multi-node deployment exactly one node runs each day's cleanup, coordinated through the shared database; no per-node configuration is needed and the xnat.is_primary_node property is not consulted. A run that has not finished by the next scheduled time stops after the launch group it is working on and continues on the following run, so a large backlog is reclaimed over several days rather than in one uninterrupted pass. Note that only build folders for containers finalized within the past 365 days are considered, so directories older than that, or already unreferenced on disk, are not removed.
