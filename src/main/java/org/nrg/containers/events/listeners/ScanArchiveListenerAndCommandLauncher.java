...
public class ScanArchiveListenerAndCommandLauncher implements Consumer<Event<ScanArchiveEventToLaunchCommands>> {
    ...
    public void accept(Event<ScanArchiveEventToLaunchCommands> event) {
        QueueUtils.sendJmsRequest(template, ScanArchiveEventToLaunchCommands.QUEUE, event.getData());
    }

    @JmsListener(containerFactory = ContainersConfig.EVENT_HANDLING_QUEUE_LISTENER_FACTORY,
                 destination = ScanArchiveEventToLaunchCommands.QUEUE)
    public void onRequest(final ScanArchiveEventToLaunchCommands scanArchiveEventToLaunchCommands) {
